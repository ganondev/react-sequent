---
sidebar_position: 1
title: Getting Started
---

# Getting Started

**react-sequent** is a lightweight React utility for building UX flows — multi-step forms, onboarding wizards, modal flows, or any UI that moves through discrete states.

The core idea: **the current step decides what comes next**, rather than a top-level config knowing all states in advance.

## Installation

```bash
npm install react-sequent
```

**Prerequisites:** React 19+

## Quick Start

A minimal two-step flow in under 40 lines:

```tsx
import { useRef } from "react";
import {
  FlowOutlet,
  useFlowInit,
  useStep,
} from "react-sequent";
import type { FlowOutletHandle } from "react-sequent";

// Step 1 — decides what to render and where to go next
function Step1() {
  const { advance } = useStep();
  return (
    <div>
      <h3>Step 1 — Welcome</h3>
      <p>This is the first step.</p>
      <button onClick={() => advance(Step2)}>Next</button>
    </div>
  );
}

// Step 2 — can go back or finish the flow
function Step2() {
  const { retreat, resolve } = useStep();
  return (
    <div>
      <h3>Step 2 — Confirm</h3>
      <button onClick={() => retreat()}>Back</button>
      <button onClick={() => resolve("completed!")}>Finish</button>
    </div>
  );
}

// Host — owns the outlet and starts the flow
function App() {
  const ref = useRef<FlowOutletHandle>(null);
  const { initFlow } = useFlowInit();

  const start = async () => {
    const result = await initFlow(Step1, ref);
    console.log("Flow resolved:", result);
  };

  return (
    <div>
      <FlowOutlet ref={ref} />
      <button onClick={start}>Start Flow</button>
    </div>
  );
}
```

### What's happening

1. **`useFlowInit`** gives you `initFlow` — call it with a step component and an outlet ref to start a flow.
2. **`<FlowOutlet>`** is where the flow renders. It's idle (invisible) until `initFlow` activates it.
3. **`useStep`** is called inside step components. It returns `advance`, `retreat`, `resolve`, and `abort` — everything a step needs to control the flow.
4. When `resolve("completed!")` is called, the promise returned by `initFlow` resolves with that value.

## Key Concepts

### Steps own transitions

There's no centralized state map. Each step calls `advance(NextStep)` to move forward. Branching is a plain `if` statement:

```tsx
function PaymentStep() {
  const { advance, resolve } = useStep();

  const onSubmit = (method: string) => {
    if (method === "credit") {
      advance(CreditCardForm);
    } else {
      advance(BankTransferForm);
    }
  };

  return <PaymentMethodPicker onSelect={onSubmit} />;
}
```

### Async steps just work

Pass a dynamic import instead of a component — Suspense handles the rest:

```tsx
const { advance } = useStep();

// Lazy-loaded step — the outlet shows the fallback while loading
advance(() => import("./HeavyStep"));
```

### Flow context

Carry data across steps without prop-drilling:

```tsx
// Start with initial context
initFlow(Step1, ref, { name: "Alice", plan: "pro" });

// In any step — read and patch context
function Step1() {
  const { advance, context } = useStep();
  return (
    <div>
      <p>Hello, {context.name}!</p>
      <button onClick={() => advance(Step2, { step1Complete: true })}>
        Next
      </button>
    </div>
  );
}
```

## Next Steps

- [**useFlowInit**](/docs/api/use-flow-init) — full reference for flow initialization
- [**useStep**](/docs/api/use-step) — transitions, context, and flow completion
- [**useFlowContext**](/docs/api/use-flow-context) — read-only context for chrome components
- [**FlowOutlet**](/docs/api/flow-outlet) — outlet props, async fallback, error boundary
