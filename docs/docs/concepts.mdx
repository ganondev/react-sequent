...
The outlet renders nothing when idle. When activated via `init()`, it renders the active step (and optional chrome). Calling `resolve()` or `abort()` tears down the flow and returns the outlet to idle.
