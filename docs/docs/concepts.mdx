...
### `defineSequentFlow` — for typed scopes

- Declares a flow's context and result types once at module scope
- Returns typed wrappers around `useSequentFlow`, `useSequentStep`, and `useSequentContext`
- Performs **no runtime work** — it is a type boundary, not a registry or provider
...
The base hooks are intentionally minimal. `useSequentStep().context` is `unknown` by default, and `advance()` accepts an untyped patch.

If you want all three hooks to agree on a single context and result type, create a typed scope with `defineSequentFlow()`:
...
- `useSequentFlow().init()` requires an initial `CheckoutContext`
- `useSequentStep().context` is `CheckoutContext`
- `useSequentStep().advance()` accepts `Partial<CheckoutContext>`
- `useSequentContext().context` is `CheckoutContext | undefined`

Typed scopes are entirely opt-in. They do not change runtime behavior or the implicit flow model.
