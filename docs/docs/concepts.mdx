...
`useStep()` is enforced at the React tree level — it throws immediately if called outside the active step's subtree (e.g. from chrome or an idle child). The correct hook for those contexts is `useFlowContext()`.
...
<FlowOutlet chrome={chrome}>   ← flow-level context (chrome + idle children)
  ...
  // chrome: can call useFlowContext(), not useStep()
  // step: can call both useStep() and useFlowContext()
</FlowOutlet>
```
