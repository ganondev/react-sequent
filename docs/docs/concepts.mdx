...
The `react-sequent/transitions` subpath ships ready-made factories — `crossfade()` and `slide()` — that turn this into a one-liner. See the [Transitions API](/docs/api/transitions) page.
...
See this exact crossfade live in the [Subsection Flow demo](/docs/demos/subsection-flow), and `slide({ fade: true })` in the [Modal demo](/docs/demos/modal).
...
For a fully custom harness built on an animation library, see the [Full-Screen Wizard demo](/docs/demos/wizard) (Motion).
