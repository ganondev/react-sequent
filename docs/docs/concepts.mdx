...
1. **`exiting`** — Both the old (exiting) and new (entering) step are mounted simultaneously. The consumer plays an exit animation and calls `onExited()` when done.
2. **`entering`** — The exiting step has unmounted. The entering step renders alone for one tick so consumers can trigger an enter animation.
...
See this exact crossfade live in the [Subsection Flow demo](/docs/demos/subsection-flow).
